...
function generate_product_details($product_id){
	...
	$details = [
		...
		'test' => $product->get_attribute('pa_test'),
	];

	return $details;	 
}

function generate_product_block($product_id, $product_details, $category_slug){
	$product = wc_get_product($product_id);
	...
	<div class="card">
		<a class="card-link link" href="<?php echo $product->get_permalink(); ?>">
            ...
            if ($tags) :
            ?>
				<div class="card-wrapper-tags">
                    <?php foreach ($tags as $tag) : ?>
						<?php
						echo '<div class="card-tag ' . $tag->slug . '">' . $tag->name . '</div>';
                        ?>
                    <?php endforeach; ?>                    
                </div>
            <?php endif; ?>
			...
			<div class="card-price-characteristics"><strong>Тест:</strong> <?php echo $product_details['test']; ?></div>
			<div class="card-price-characteristics"><strong>Длина:</strong> <?php echo $product_details['length']; ?></div>
			<div class="card-price-characteristics"><strong>Вес:</strong> <?php echo $product_details['weight']; ?></div>
			<div class="card-price-characteristics"><strong>Материал:</strong> <?php echo $product_details['material']; ?></div>
			
			
			<?php if ($product_sale_price > 0): ?>
            <p class="card-price discount">
                <span class="card-price-discount"><?php echo number_format($product_sale_price, 0, '.', ' '); ?>&nbsp;Грн</span>
                <span class="card-price-old"><?php echo number_format($product_regular_price, 0, '.', ' '); ?>&nbsp;Грн</span>
            </p>
            <?php endif; ?>
			...
			<div class="card-actions">
				<?php if($product->is_in_stock()): ?>
					<a href="/cart/?add-to-cart=<?php echo $product_id; ?>" class="add-to-cart-button page-cursor-pointer">В корзину</a>				
				<?php else: ?>
				<div role="button" tabindex="0" class="add-to-cart-button page-cursor-pointer no-available">
...
function generate_owl_cards_section($category_slug) {
	...
	<section class="main-section main-section-<?php echo $category_slug; ?>">
		<div class="container main-section-container">
			...
			echo '<a class="main-section-link" href="/'.$category_slug.'"><span class="main-section-link">ПЕРЕЙТИ В РАЗДЕЛ</span></a>';
			?>
			<div class="cards-container">
			<?php
				$args = array(
